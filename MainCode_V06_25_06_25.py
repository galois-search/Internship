...
if __name__ == "__main__":
    ...
    for item in result:
        ...
        for ele in binary_sequence:
            sequence += str(ele)
        all_sequence.append((sequence,power_x0,power_mu,period,acr_val,balance_val))

    grouped = DuplicateSequence.group_duplicates_by_original(all_sequence)

    #Checking for decimation equivalent
    for key,value in grouped.values():
        decimation = Decimation.generate_sequences_from_sequence_v3(key)
        uniq = Algorithm2.find_unique_sequences_v4(decimation)
        common_values = DuplicateSequence.check_sequences_in_group_files(decimation,value)
        with open(f"common_decimation_values_{key}.txt",'w') as k:
            for val in common_values:
                k.write(f"power_x0 : {val[1]}, power_mu : {val[2]}, period : {val[3]}, acr_val : {val[4]}, balance : {val[5]}, sequence : {val[0]}\n")

    #shifts of the original
    for original, duplicates in grouped.values():
        if duplicates:  # Only create file if there are duplicates
            for element in duplicates:
                if element[2] == 6:
                    filename = f"original_{original}_{element[2]}_duplicates.txt"
                    with open(filename, 'a') as f:
                        f.write(f"power_x0 : {element[1]}, power_mu : {element[2]}, period : {element[3]}, acr_val : {element[4]}, balance : {element[5]}, sequence : {element[0]}\n")
                else:
                    filename = f"original_{original}_{element[2]}_duplicates.txt"
                    with open(filename, 'a') as f:
                        f.write(
                            f"power_x0 : {element[1]}, power_mu : {element[2]}, period : {element[3]}, acr_val : {element[4]}, balance : {element[5]}, sequence : {element[0]}\n")
