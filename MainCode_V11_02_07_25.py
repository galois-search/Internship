...
def main(input_file_path,input_file_name,input_acr_value,input_balance_value):
    ...
    for element in unique_sequence_before_decimation:
        ...
        for val in decimated_values:
            dec_no += 1
            decimation_of_unique_sequence.append([val, element[1], seq_no, dec_no])
    ...
    with open(path_decimation, 'w') as of:
        for value in decimation_of_unique_sequence:
            ...
            balance = Acr_Bal_Program.calculate_balance(seq)
            if acr <= input_acr_value and balance <= abs(input_balance_value):
                of.write(f"sequence no : S{value[2]},power_x0 : {value[1][0]},power_mu : {value[1][1]},period : {len(original_seq)},acr_val : {acr},balance : {balance},sequence : {original_seq}, decimation no : d{value[3]}\n")
    ...
    with open(path_unique_after_dec, 'w') as output_file:
        ...
        for value in unique_sequence_after_decimation:
            ...
            if acr <= input_acr_value and balance <= abs(input_balance_value):
                D_U_C += 1
                output_file.write(f"sequence no : S{value[2]},power_x0 : {value[1][0]},power_mu : {value[1][1]},period : {len(original_seq)},acr_val : {acr},balance : {balance},sequence : {original_seq}, decimation no : d{value[3]}\n")
